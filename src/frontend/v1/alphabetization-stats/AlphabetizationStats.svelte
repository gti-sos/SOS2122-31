<script>
    ...
    async function getReg(){
		...
		let c = `/api/v1/alphabetization-stats?limit=${limit}&&offset=${offset*10}&&`;
		if (from != null) {
			c = c + `from=${from}&&`
		}
		if (to != null) {
	...
	async function BorrarRegis(countryDelete, yearDelete){
        const res = await fetch("/api/v1/alphabetization-stats/"+countryDelete+"/"+yearDelete,
			{
				method: "DELETE"
			}).then(function(res){
				if(res.ok){
			...
			<Table bordered>
				...
				<tbody>
					<tr>
						...
						<td><input type="number" bind:value="{newRegistration.ar_ym}"></td>
						<td><input type="number" bind:value="{newRegistration.ar_yw}"></td>
						<td><input type="number" bind:value="{newRegistration.ar_ty}"></td>
						<td><Button 
							outline 
							color="primary"
